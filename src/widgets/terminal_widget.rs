use crate::{BottomEvent, app::App};
use serde::__private::from_utf8_lossy;
use std::{collections::VecDeque, sync::{mpsc::Sender, Mutex, MutexGuard}};
...
impl UnsafeTerminalWidgetState {
	...
	fn get_tws<'a>(&self, app_lock: &'a mut MutexGuard<'_, Option<App>>) -> &'a mut TerminalWidgetState {
		let app = app_lock.as_mut().unwrap();
		let current_widget_id = app.current_widget.widget_id;
		app
			.terminal_state
			.widget_states
			.get_mut(&current_widget_id).unwrap()
	}
